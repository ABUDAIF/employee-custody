...
import { FaTimes, FaSearch, FaWallet, FaUser, FaArrowRight } from 'react-icons/fa'
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <h3 style={{ fontSize: '18px', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <FaWallet color="var(--accent-success)" /> تفاصيل الأرصدة المتبقية لكل موظف
            </h3>
            <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
              إجمالي الأرصدة المتبقية لدى الموظفين حالياً: <strong style={{ color: 'var(--accent-success)' }}>{totalAllRemaining.toLocaleString('ar-EG')} ج.م</strong>
            </span>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-muted)', fontSize: '12px' }}>
                ...
                <th style={{ padding: '10px' }}>الرصيد المتبقي الحالي (ج.م)</th>
            ...
            <tbody>
              {filtered.map((emp) => (
                <tr key={emp.id} style={{ borderBottom: '1px solid var(--border-subtle)', fontSize: '13px' }}>
                  ...
                  <td style={{ padding: '12px', color: 'var(--accent-brand)' }}>
                  ...
                  <td style={{ padding: '12px', fontWeight: '800', fontSize: '15px', color: 'var(--accent-success)' }}>
