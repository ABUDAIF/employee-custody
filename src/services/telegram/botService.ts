          ...
          case 'AWAITING_ATTACHMENT': {
            ...
            if (fileId) {
              try {
                ...
                await this.bot?.sendMessage(chatId, '⚠️ حدث خطأ أثناء تحميل الفاتورة، سيتم حفظ المصروف بدون الفاتورة.')
                await this.finalizeExpense(chatId, telegramId, employee, session)
              }
            } else {
              await this.bot?.sendMessage(chatId, '❌ لم نتمكن من التعرف على الملف. يرجى إرسال صورة أو ملف PDF أو الضغط على إلغاء.')
            }
            break
          }
    ...
    try {
      ...
      await this.bot?.sendMessage(
        ...
        `✅ **تم تسجيل المصروف بنجاح!**\n\n` +
          ...
          (entry.attachments.length > 0 ? `📎 **المرفقات:** ${entry.attachments.length} ملف\n` : '') +
          `----------------------------------\n` +
